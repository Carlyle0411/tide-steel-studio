...
export function StoryboardWorkspaceView() {
  ...
  useEffect(() => {
    let alive = true;
    const refresh = async () => { const store = await loadKeyframeStore(); if (alive) setKeyframes(store); };
    void refresh();
  ...
  function save() {
    ...
    setMessage("Storyboard已保存，Camera设计将读取同一份镜头数据。");
  }
  ...
  function mergePrevious() {
    ...
    const merged = { ...previous, title: `${previous.title} / ${selected.title}`, description: `${previous.description}\n${selected.description}`, duration: previous.duration + selected.duration, notes: [previous.notes, selected.notes].filter(Boolean).join("\n"), updatedAt: new Date().toISOString() };
    const next = [...shots];
  ...
  function exportJson() {
    const url = URL.createObjectURL(new Blob([JSON.stringify({ episode: "EP01", exportedAt: new Date().toISOString(), shots }, null, 2)], { type: "application/json" }));
  ...
  return (
    <div className="space-y-4">
      <ProductionCard className="flex flex-wrap items-end justify-between gap-3 p-5">
        <div><div className="text-xs uppercase tracking-[0.22em] text-jade/80">EP01 Storyboard</div><h2 className="mt-2 text-2xl font-semibold text-white">分镜设计</h2><p className="mt-2 text-sm text-slate-400">{shots.length}个可编辑镜头，绑定关键帧、摄影、声音、对白与审核状态。拖动左侧镜头可真实排序。</p></div>
        <div className="flex gap-2"><button className="btn h-9" onClick={exportJson}><Download size={14} /> 导出JSON</button><button className="btn h-9 border-jade text-jade" onClick={save}><Save size={14} /> 保存Storyboard</button></div>
      </ProductionCard>
      <div className="grid gap-4 xl:grid-cols-[380px_minmax(0,1fr)]">
        ...
        {selected && <ProductionCard className="p-5">
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
            <div>
              ...
              <div className="grid gap-3 md:grid-cols-3"><Field label="时长（秒）" value={String(selected.duration)} onChange={(value) => update("duration", Math.max(1, Number(value) || 1))} /><Field label="景别" value={selected.shotSize} onChange={(value) => update("shotSize", value)} /><Field label="焦段" value={selected.lens} onChange={(value) => update("lens", value)} /></div>
              <div className="grid gap-3 md:grid-cols-2"><Field label="摄影机位置" value={selected.camera} onChange={(value) => update("camera", value)} /><Field label="运镜" value={selected.movement} onChange={(value) => update("movement", value)} /></div>
              <div className="grid gap-3 md:grid-cols-2"><Field label="角色" value={selected.character} onChange={(value) => update("character", value)} /><Field label="场景" value={selected.environment} onChange={(value) => update("environment", value)} /></div>
              <div className="grid gap-3 md:grid-cols-2"><Field label="光线" value={selected.lighting} onChange={(value) => update("lighting", value)} /><Field label="情绪" value={selected.emotion} onChange={(value) => update("emotion", value)} /></div>
              <Field label="声音" value={selected.sound} multiline onChange={(value) => update("sound", value)} /><div className="grid gap-3 md:grid-cols-2"><Field label="对白" value={selected.dialogue} multiline onChange={(value) => update("dialogue", value)} /><Field label="音乐" value={selected.music} multiline onChange={(value) => update("music", value)} /></div><Field label="导演备注" value={selected.notes} multiline onChange={(value) => update("notes", value)} />
            </div>
            <div>
              <div className="aspect-video overflow-hidden rounded border border-white/10 bg-white/[0.03]">{best ? <img src={best.dataUrl} alt={selected.title} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-sm text-slate-500">等待上传 {selected.keyframeId}</div>}</div>
              <label className="mt-3 block text-xs text-slate-500">制作状态<select className="mt-2 h-10 w-full rounded border border-white/10 bg-[#0b1016] px-3 text-sm text-slate-200" value={selected.status} onChange={(event) => update("status", event.target.value as StoryboardStatus)}>{statuses.map((status) => <option key={status}>{status}</option>)}</select></label>
              <div className="mt-4 grid gap-2"><button className="btn h-9" onClick={duplicate}><Copy size={14} /> 复制Shot</button><button className="btn h-9" onClick={split}><Scissors size={14} /> 拆分Shot</button><button className="btn h-9" disabled={selectedIndex <= 0} onClick={mergePrevious}><GitMerge size={14} /> 与上一Shot合并</button></div>
            </div>
          </div>
...
function Field({ label, value, multiline = false, onChange }: { label: string; value: string; multiline?: boolean; onChange: (value: string) => void }) {
  return <label className="mt-3 block"><span className="text-[10px] uppercase tracking-[0.16em] text-slate-500">{label}</span>{multiline ? <textarea className="mt-2 min-h-24 w-full resize-y rounded border border-white/10 bg-black/30 p-3 text-sm leading-6 text-slate-200 outline-none" value={value} onChange={(event) => onChange(event.target.value)} /> : <input className="mt-2 h-10 w-full rounded border border-white/10 bg-black/30 px-3 text-sm text-slate-200 outline-none" value={value} onChange={(event) => onChange(event.target.value)} />}</label>;
}
