...
import { trilogyScriptDocuments } from "../../../mcp/trilogy/TrilogyStoryData";
...
const STORAGE_KEY = "tide-steel-soul-script-workspace-trilogy-v1";
const documents = trilogyScriptDocuments;
...
export function ScriptWorkspaceView({ navigate }: { navigate: (section: ProductionSection) => void }) {
  ...
  function restore() {
    const ok = window.confirm(`确定恢复“${doc.label}”的三部曲原文吗？当前浏览器里的修改会被清除。`);
    if (!ok) return;
    ...
    <div className="space-y-4">
        ...
        <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-400">
              这里只保留《潮汐钢魂》三部曲正式结构、第一部12集弧线和90秒预告片规划。旧版单集电影和早期EP01内容不再进入网页。
            </p>
          </div>
      ...
      <ProductionCard className="flex flex-wrap items-center justify-between gap-3 p-3">
        <div className="flex flex-wrap gap-2">
          {documents.map((item) => (
            <button key={item.id} className={`btn h-10 ${item.id === doc.id ? "border-jade bg-jade/10 text-jade" : ""}`} onClick={() => selectDocument(item.id)}>
              {item.label}
            </button>
          ))}
        </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[280px_minmax(0,1fr)]">
        <ProductionCard className="max-h-[760px] overflow-y-auto p-3">
          ...
          <div className="grid gap-1">
            {sections.map((section) => (
              <button
                key={section.id}
                className={`rounded border px-3 py-2 text-left text-sm ${active?.id === section.id ? "border-jade bg-jade/10 text-jade" : "border-white/10 text-slate-300 hover:border-jade/40"}`}
                onClick={() => { setSectionId(section.id); setMessage(""); }}
              >
        ...
        <ProductionCard className="p-4">
          ...
          <textarea
            className="min-h-[680px] w-full resize-y rounded border border-white/10 bg-black/30 p-4 font-mono text-sm leading-7 text-slate-200 outline-none"
