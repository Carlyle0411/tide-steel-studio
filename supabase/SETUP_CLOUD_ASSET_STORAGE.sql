-- Tide Steel Studio: private multi-device asset library
...
create policy "tide_assets_select_own" on storage.objects
  ...
  using (bucket_id = 'tide-assets' and owner_id = (select auth.uid()));
...
create policy "tide_assets_insert_own" on storage.objects
  ...
  with check (bucket_id = 'tide-assets' and owner_id = (select auth.uid()));
